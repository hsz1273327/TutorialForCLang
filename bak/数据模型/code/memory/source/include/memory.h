 ...
 * @param [s2]             const void *      要对比的第二段内存指针
 ...
 * @param [s2]             const void *  要复制的对象指针
 * @param [n]              size_t        要复制的内存块数
 * @return                 void*         复制内容到的地址指针
 * @brief                                将内存从一段内存移动到另一段内存
 * @detial      将内存从一段内存复制到另一段内存,两段内存可以有重叠
 */
 ...
 * @detial
 * 复制指定对象指针上的内容并重新分配一块指定大小的空间给这个对象,不同的实现会有不同的内存检测
 */
