...
我们可以在`target_sources`,`target_include_directories`,`target_link_libraries`等用于描述目标使用的资源的方法中设置访问可见性标识.标识的范围有
...
我们可以看例子中的`CMakeLists.txt_withshare`文件,它演示了允许同时生成两种库的模式:
