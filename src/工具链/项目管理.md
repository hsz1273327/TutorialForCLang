...
cmake默认的后端是makefile,但我们也可以通过`-G`设置后端,常用的后端还有[Ninja](https://github.com/ninja-build/ninja/releases)和`MinGW Makefiles`等.为了跨平台通用性本文以Ninja为例子.实际上一旦cmake写好后生成什么后端并不重要,生成了配置文件后只要执行它就可以了.
