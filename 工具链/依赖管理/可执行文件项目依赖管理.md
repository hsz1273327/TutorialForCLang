...
1. 设置一些必要的环境变量和设置(根据平台情况设置)
    ...
    conan则主要要设置它名为`default`的`profile`.所谓`profile`可以理解为编译环境设置,通过`conan profile detect`创建,通过`conan profile update <key>=<value> <profile_name>`设置.默认位置在`~/.conan2/profiles/default`文件中保存
