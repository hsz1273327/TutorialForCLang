...
static void cvt_s(int code, va_list_box *box,
	...
	assert(str);
	Format_puts(str, strlen(str), put, cl, flags,
		width, precision);
}
static void cvt_d(int code, va_list_box *box,
	...
	if (val < 0)
		*--p = '-';
	Format_putd(p, (buf + sizeof buf) - p, put, cl, flags,
		width, precision);
}
...
static void cvt_f(int code, va_list_box *box,
	...
	Format_putd(buf, strlen(buf), put, cl, flags,
		width, precision);
}
const Except_T Format_Overflow = { "Formatting Overflow" };
static T cvt[256] = {
 /*   0-  7 */ 0,     0, 0,     0,     0,     0,     0,     0,
...
void Format_puts(const char *str, int len,
	int put(int c, void *cl), void *cl,
	unsigned char flags[], int width, int precision) {
	assert(str);
	assert(len >= 0);
...
void Format_fmt(int put(int c, void *), void *cl,
	const char *fmt, ...) {
	va_list_box box;
	va_start(box.ap, fmt);
	Format_vfmt(put, cl, fmt, &box);
	va_end(box.ap);
}
void Format_print(const char *fmt, ...) {
	va_list_box box;
	va_start(box.ap, fmt);
	Format_vfmt(outc, stdout, fmt, &box);
	va_end(box.ap);
}
void Format_fprint(FILE *stream, const char *fmt, ...) {
	va_list_box box;
	va_start(box.ap, fmt);
	Format_vfmt(outc, stream, fmt, &box);
	va_end(box.ap);
}
int Format_sfmt(char *buf, int size, const char *fmt, ...) {
	int len;
	va_list_box box;
	va_start(box.ap, fmt);
	len = Format_vsfmt(buf, size, fmt, &box);
	va_end(box.ap);
	return len;
}
int Format_vsfmt(char *buf, int size, const char *fmt,
	va_list_box *box) {
	struct buf cl;
	assert(buf);
	assert(size > 0);
...
char *Format_string(const char *fmt, ...) {
	...
	assert(fmt);	
	va_start(box.ap, fmt);
	str = Format_vstring(fmt, &box);
	va_end(box.ap);
	return str;
}
char *Format_vstring(const char *fmt, va_list_box *box) {
	struct buf cl;
	assert(fmt);
	cl.size = 256;
	cl.buf = cl.bp = ALLOC(cl.size);
	Format_vfmt(append, &cl, fmt, box);
	append(0, &cl);
	return RESIZE(cl.buf, cl.bp - cl.buf);
}
void Format_vfmt(int put(int c, void *cl), void *cl,
	...
	while (*fmt)
		if (*fmt != '%' || *++fmt == '%')
		...
		else
			{
				...
				if (Format_flags) {
					unsigned char c = *fmt;
					for ( ; c && strchr(Format_flags, c); c = *++fmt) {
						assert(flags[c] < 255);
						flags[c]++;
					}
				}
...
T Format_register(int code, T newcvt) {
	T old;
	assert(0 < code
...
void Format_putd(const char *str, int len,
	int put(int c, void *cl), void *cl,
	unsigned char flags[], int width, int precision) {
	int sign;
	assert(str);
